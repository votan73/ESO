...
local function FindAndEquipItems(itemsToFindAndEquip)
	...
	if next(itemsToFindAndEquip) == nil then return end
	
	local backpackSlots = SHARED_INVENTORY:GetOrCreateBagCache(BAG_BACKPACK)-- PLAYER_INVENTORY.inventories[INVENTORY_BACKPACK].slots
	local next = next
...
local function UnequipUnusedSlots(equipSlotsToUnequip, itemsToFindAndEquip)
	...
	if slotHasItem then
		...
		if CheckInventorySpaceSilently(1) then
			...
			EVENT_MANAGER:RegisterForUpdate("LIBSWAP_FindAndEquipItems", BUFFER_TIME_UNEQUIP_LOAD_DELAY, function()
...
function libswp:SaveEquipmentSet()
	if libswp.equipProcessInProgress then return false end
...
function libswp:LoadEquipmentSet(equipmentSet)
	...
	if next(equipSlotsToUnequip) ~= nil then
		EVENT_MANAGER:RegisterForUpdate("LIBSWAP_UnequipUnusedSlots", 150, function()
...
function libswp:SaveSkillSet()
	if not libswp.skillSetAllowSave then return false end
...
function libswp:LoadSkillSet(skillBarData)
	if type(skillBarData) ~= "table" then return false end
	if not libswp.skillSetAllowLoad then return false end
	libswp.skillSetAllowLoad = false
	libswp.skillSetAllowSave = false
	...
	EVENT_MANAGER:RegisterForUpdate("LIBSWAP_BufferLoadSkillBar", 50, function()
