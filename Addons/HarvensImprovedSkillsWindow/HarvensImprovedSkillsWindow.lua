...
local function AddInfo(tooltip, progressionData, skillRank)
    ...
    if not completed then
        ...
        tooltip:AddLine(GetString(SI_ABILITYPROGRESSIONRESULT1), "ZoFontWinH5", r, g, b, TOPLEFT, MODIFY_TEXT_TYPE_NONE,
            TEXT_ALIGN_CENTER, SET_TO_FULL_SIZE)
    end
end
    ...
    if not skillData:IsCraftedAbility() then
        if not skillData:IsPassive() then
            ...
            elseif skillData:IsPassive() then
                ...
                InitializeTooltip(HarvensSkillTooltipMorph2, control, TOPRIGHT, -5, -5, TOPLEFT)
                HarvensSkillTooltipMorph2:SetSkillLineAbilityId(skillData:GetProgressionData(2):GetAbilityId(), skillType,
                    skillLineIndex, skillIndex, 2)
                InitializeTooltip(HarvensSkillTooltipMorph1, HarvensSkillTooltipMorph2, TOPRIGHT, -5, 0, TOPLEFT)
                HarvensSkillTooltipMorph1:SetSkillLineAbilityId(skillData:GetProgressionData(1):GetAbilityId(), skillType,
                    skillLineIndex, skillIndex, 1)
...
function HarvensImprovedSkillsWindow:Initialize()
    ...
    local function createCheckbox(name, anchor, label, setting)
        local checkbox = WINDOW_MANAGER:CreateControlFromVirtual(SKILLS_WINDOW.control:GetName() .. name,
            SKILLS_WINDOW.control, "HarvensImprovedSkillsWindowShowDetails")
    ...
    if PP then
        createCheckbox("HarvensShowDetails",
            { BOTTOMRIGHT, SKILLS_WINDOW.control, TOPRIGHT, -8, -65, ANCHOR_CONSTRAINS_XY },
            "Show detailed skills progression", "showDetails")
    else
        createCheckbox("HarvensShowDetails",
            { BOTTOMRIGHT, SKILLS_WINDOW.control, TOPRIGHT, -8, -70, ANCHOR_CONSTRAINS_XY },
            "Show detailed skills progression", "showDetails")
    end
    createCheckbox("HarvensShowTotal",
        { BOTTOMRIGHT, SKILLS_WINDOW.control:GetNamedChild("HarvensShowDetails"), TOPRIGHT, 0, -8 },
        "Show total skill line progression", "showTotal")

    SecurePostHook(_G, "ZO_Skills_AbilitySlot_OnMouseEnter",
        function(control)
                ...
                if not skillData:IsPassive() then
                    ...
                    if progressionIndex and progressionIndex > 0 then
                        ...
                        if curLvl and maxLvl and curLvl < maxLvl then
                            ...
                            InitializeTooltip(HarvensSkillTooltipMorph2, control, TOPRIGHT, -5, -5, TOPLEFT)
                            HarvensSkillTooltipMorph2:SetSkillLineAbilityId(
                                skillData:GetProgressionData(2):GetAbilityId(), skillType, skillLineIndex, skillIndex)
                            InitializeTooltip(HarvensSkillTooltipMorph1, HarvensSkillTooltipMorph2, TOPRIGHT, -5, 0,
                                TOPLEFT)
                            HarvensSkillTooltipMorph1:SetSkillLineAbilityId(
                                skillData:GetProgressionData(1):GetAbilityId(), skillType, skillLineIndex, skillIndex)
                        end
                    end
                else
                    local maxRank = skillData:GetNumRanks()
    ...
    SecurePostHook(_G, "ZO_Skills_AbilitySlot_OnMouseExit",
        function()
            ClearTooltip(HarvensSkillTooltipMorph1)
            ClearTooltip(HarvensSkillTooltipMorph2)
        end)
    ...
    local function createLabel(ctrl)
        ...
        ctrl.label = WINDOW_MANAGER:CreateControlFromVirtual("$(parent)HarvensLabel", ctrl,
            "HarvensImprovedSkillsWindowLabel")
        ctrl.label:SetAnchor(CENTER, ctrl, CENTER, 0, 3)
    end

    local function createMorph(control, ctrl)
        control.morphLabel = WINDOW_MANAGER:CreateControlFromVirtual("$(parent)HarvensMorphLabel", ctrl,
            "HarvensImprovedSkillsWindowMorphLabel")
    ...
    local function applyInfo(control, skillData)
        ...
        if not skillData:IsPassive() then
            local hasprog, progId, lastXP, nextXP, currentXP, atMorph = GetAbilityProgressionXPInfoFromAbilityId(
                skillProgressionData:GetAbilityId())
    ...
    function SKILLS_WINDOW:RefreshSkillLineInfo(...)
        ...
        if nextXP > 0 and nextXP > currentXP then
            ...
            if HarvensImprovedSkillsWindow.sv.showTotal and maxRank > 0 then
                text[#text + 1] = "  " .. string.format("%i/%i", lineRank, maxRank)
            end
            ctrl.label:SetText(table.concat(text))
            ctrl.label:SetHidden(false)
        else
